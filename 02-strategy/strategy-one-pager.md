...
The Problem: roadmap discussions at RocketShip are driven by the loudest voice in Teams Channel rather than customer evidence. Priorities reverse weekly; stakeholder trust is eroding.
...
Approach: Ground (RAG). We will ground the model in the RocketShip corpus - Teams Channel #escalations, Teams Channel #hypercare, support tickets, interview notes, Confluence product pages, Jira tickets - so every priority cites a source ID.
...
Mitigation: a hard 'evidence balance' eval gate - reject any priority list where more than 50% of cited sources come from a single source type. Run weekly; PM reviews.

## 6. V1 Scope

In: ranking the existing backlog with cited evidence; surfacing under-cited items; flagging conflicts between Teams Channel escalations and Jira priorities.

Out: (1) hiring or headcount decisions, (2) customer-facing comms about why a feature was deprioritized. Both stay 100% with the human PM.
