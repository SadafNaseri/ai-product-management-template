...
The Problem: roadmap discussions at RocketShip are driven by the loudest voice in Teams Channels rather than customer evidence. Priorities reverse weekly; stakeholder trust is eroding.
...
Approach: Ground (RAG). We will ground the model in the RocketShip corpus - Team Channels #escalations, Team Channels #hypercare, support tickets, interview notes, Confluence product pages, Jira tickets - so every priority cites a source ID.
...
In: ranking the existing backlog with cited evidence; surfacing under-cited items; flagging conflicts between Team Channel escalations and Jira priorities.
