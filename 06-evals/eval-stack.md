...
| Human | 06-evals/human-rubric.md · 2 graders + PM tiebreak per disagreement protocol · cadence: Weekly batch (Friday afternoon) | - 50 P0 triage runs / week - Stratified across confidence buckets (high >= 80%, mid 70-79%, low < 70%) - 100% of hand-off cases included | >=4.0/5 mean across accuracy + safety; 0 critical safety fails |
...
- Cost per run > $0.50 (auto-block; matches README red line and Agent Control Panel cost cap)
...
- p95 latency >8s requires PM justification
